...
<style>
    ...
    /** level 2 */
    .cmfive-comment>.cmfive-comment .comment-body {
        border-left: 10px solid #3F7CAC;
    }

    /** level 3 */
    .cmfive-comment>.cmfive-comment>.cmfive-comment .comment-body {
        border-left: 10px solid #95AFBA;
    }

    /** level 4 */
    .cmfive-comment>.cmfive-comment>.cmfive-comment>.cmfive-comment .comment-body {
        border-left: 10px solid #FFCAB1;
    }

    /** level 5 */
    .cmfive-comment>.cmfive-comment>.cmfive-comment>.cmfive-comment>.cmfive-comment .comment-body {
        border-left: 10px solid #393D3F;
    }

    /** level 6 */
    .cmfive-comment>.cmfive-comment>.cmfive-comment>.cmfive-comment>.cmfive-comment>.cmfive-comment .comment-body {
...
if (AuthService::getInstance($w)->hasRole('comment')) {
        ...
        <div id="<?php echo $internal_only === true ? 'internal' : 'external'; ?>_comments_container">
            <?php echo $w->partial(
                ...
                [
                    "object" => $comments,
                    "redirect" => $redirect,
                    'internal_only' => (isset($internal_only) ? $internal_only : false),
                    'external_only' => (isset($external_only) ? $external_only : false)
                ],
                "admin"
            ); ?>
        </div>
    ...
    ?>
    <div class="warning">
        <p>You do not have permissions to view and make comments. See your system administrator.</p>
    </div>
    <?php
}
