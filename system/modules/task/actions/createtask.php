    ...
    if ($taskgroups) {
        foreach ($taskgroups as $tgroup) {
            if ($tgroup->getCanICreate()) {
                $mytaskgroups[] = [$tgroup->title, $tgroup->id];
            }
        }
        ...
        if (Request::mixed(!empty($key) ? $key : null) != "") {
            ...
            $tasktypes = ($t != "") ? TaskService::getInstance($w)->getTaskTypes($t->task_group_type) : [];
            $priority = ($t != "") ? TaskService::getInstance($w)->getTaskPriority($t->task_group_type) : [];
            $members = ($t != "") ? TaskService::getInstance($w)->getMembersBeAssigned($t->id) : [];
            sort($members);
        ...
        $f = HtmlBootstrap5::form([
            ["Create a New Task - Step One", "section"],
            ["Task Group", "select", "task_group_id", Request::int('gid'), $mytaskgroups],
            ["Task Title", "text", "title"],
            ["Task Type", "select", "task_type", null, $tasktypes],
            ["Priority", "select", "priority", null, $priority],
            ["Date Due", "date", "dt_due"],
            ["Description", "textarea", "description", null, 110, 15],
            ["Assigned To", "select", "first_assignee_id", null, $members],
    ...
    if ($task->id) {
        foreach ($_POST as $name => $value) {
            if (($name != "formone") && ($name != "FLOW_SID") && ($name != "task_id") && ($name !== CSRF::getTokenID())) {
                ...
                $arr = ["task_id" => $task->id, "key" => $name, "value" => $value];
                $tdata->fill($arr);
                $tdata->insert();
                unset($arr);
            }
