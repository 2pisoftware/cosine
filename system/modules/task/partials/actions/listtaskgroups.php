...
function listtaskgroups(\Web $w, $params = [])
{
    ...
    if ($should_filter) {
        $taskgroups = array_filter($taskgroups, function ($taskgroup) use ($filter_closed_tasks) {
            ...
            if (count($tasks) == 0) {
                ...
                $tasks = array_filter($tasks, function ($task) use ($filter_closed_tasks) {
