...
class TicketEmailProcessor extends ProcessorType
{
    ...
    public function getSettingsForm($current_settings = null, $w = null): array
    {
        // Check if json
        ...
        return ["Settings" => [
            ...
            [["Authenticate as", "autocomplete", "authenticate_as", @$current_settings->authenticate_as, !empty($w) ? AuthService::getInstance($w)->getUsers() : []]],
    ...
    public function process($processor)
    {
        ...
        $messages = ChannelService::getInstance($processor->w)->getNewOrFailedMessages($processor->channel_id, $processor->id);
        if (!empty($messages)) {
            foreach ($messages as $message) {
                ...
                if ($new_task) {
                    ...
                    $task = TaskService::getInstance($processor->w)->createTask(
                        task_type: TaskTicketService::getInstance($processor->w)->task_type,
                        task_group_id: $support_taskgroup->id,
                        title: $subject,
                        description: "From: $from<br/>\nBody: $body",
                        priority: "Normal",
                        dt_due: null,
                        first_assignee_id: $support_taskgroup->default_assignee_id
                    );
                    ...
                    if (!empty($support_template->id)) {
                        ...
                        if (!empty($contact->id)) {
                            ...
                            $template_body = TemplateService::getInstance($processor->w)->render($support_template, ["task" => $task, "contact" => $contact, "email" => $email]);
                        } else {
                            $template_body = TemplateService::getInstance($processor->w)->render($support_template, ["task" => $task, "email" => $email]);
                        }
